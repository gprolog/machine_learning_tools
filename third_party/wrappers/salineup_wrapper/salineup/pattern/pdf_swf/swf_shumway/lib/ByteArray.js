...
;var ByteArray = ByteArray || (function (undefined) {
  ...
  BAp.writeByte = function writeByte(v) {
    //print("[HOOK] ByteArray.writeByte");
    controller.byteArrayMonitor.writeByte(v);
  ...
  BAp.writeBytes = function writeBytes(bytes, offset, length) {
    //print("[HOOK] ByteArray.writeBytes");
    if (g_isDoSwf == false){
        controller.byteArrayMonitor.writeBytes(bytes);
    }
  ...
  //BAp.writeUTFBytes = function writeUTFBytes(bytes) {
  //  print("[HOOK] ByteArray.writeUTFBytes");
  //}
  ...
  BAp.writeInt = function writeInt(v) {
    //print("[HOOK] ByteArray.writeInt");
    controller.byteArrayMonitor.writeInt(v);
  ...
  BAp.writeUnsignedInt = function writeUnsignedInt(v) {
    //print("[HOOK] ByteArray.writeUnsignedInt");
    if (g_isDoSwf == false){
    	controller.byteArrayMonitor.writeUnsignedInt(v);
    }
    set(this, 'setUint32', 4, v);
  };
  BAp.writeUnsignedIntInner = function writeUnsignedInt(v) {
    set(this, 'setUint32', 4, v);
  };
  ...
  BAp.uncompress = function (algorithm) {
    ...
    switch (algorithm) {
      case 'zlib':
        // add try ... catch here, make sure it could be resumed when uncompress failed
        try {

          var inflate = new Zlib.Inflate(this.uint8v);
          var uncompressed = inflate.decompress();
          this.ensureCapacity(uncompressed.length);
          this.uint8v.set(uncompressed);
          this.length = uncompressed.length;
          this.position = 0;
          return;
