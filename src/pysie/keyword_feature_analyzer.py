...
class KeywordFeatureAnalyzer:
    ...
    def __init__(self, config, usemaxtf):
        self.config_ = config
        ...
        self.usetfidf = 0
        self.config_tfidf_keywords = OrderedDict(self.config_["feature_extraction"]["tfidf_keywords"])
    
    def analyze_content_tfidf(self, content, top_n):
        ...
        ordered_keywords_list = sorted(self.config_tfidf_keywords.items(), key=lambda item: item[1], reverse=True)
        #ordered_keywords_list = self.config_tfidf_keywords.items()
        ...
        for word in set(keywords):
            if word in keylist:
                self.keyword_features_index_value_[config_keyword_index[word]] = self.config_tfidf_keywords[word] 
