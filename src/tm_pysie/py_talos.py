    ...
    for root, dir, samples in os.walk(samplepath):
        for sample in samples:
            ...
            with open(sample, 'r+') as fd:
                content = fd.read()
                if usemaxtf:
		    kf.analyze_content_tfidf(content, top_n)
		else:
...
def GetMatchFeature_MultiProcess(dllpath, samplepath_list, out_path, sample_label, config_path, top_n,  usetfidf):
    tmsie = TmsieWrapper(dllpath)
    with open(config_path, 'rb') as fh:
        config = json.load(fh, object_pairs_hook=OrderedDict)
    kf = KeywordFeatureAnalyzer(config, usetfidf)
    for sample in samplepath_list:
        if True:
            feature_word = ""
            with open(sample, 'r+') as fd:
                content = fd.read()
...
def add_option(option_parser):
    ...
    option_parser.add_option("--usetfidf", dest='usetfidf', help="usetfidf", type="string")
...
if __name__ == "__main__":
    ...
    if not dll_path:
        if platform.system() == 'Windows':
            ...
            if platform.architecture()[0] == '64bit':
                dll_path = os.path.join('build', 'jsvbs', 'build', 'libtmsie.so')
            else:
    ...
    for i in range(0, proc_num):
        out = out_path + '_' + str(i) + '.txt'
        proc_list.append(multiprocessing.Process(target=GetMatchFeature_MultiProcess, args=(dll_path, file_list_map[i], out, sample_label, config, start_index, usetfidf)))
...
'''
    proclist = []
    for i in range(start_index, start_index+interval, interval):
        out = out_path + '_' + str(i) + '.txt'
