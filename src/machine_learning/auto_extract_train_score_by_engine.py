...
for topn in range(100, 2000, 200):
    # 3. extract feature
    working_dir = os.path.join(os.getcwd(),'tm_pysie','build','x64')
    DLL_PATH = os.path.join(working_dir, 'libtmsie.so')
    sie_config_path = os.path.islink(os.getcwd(), 'pysie', 'pysie.cfg')
    os.chdir(working_dir)
    cmd = 'python ../../py_talos.py --dllpath="{}" --label=1 --samplepath="{}" --out="{}" --configpath="{}" --topn="{}"'.format(DLL_PATH, MAL_SAMPLE_DIR, MAL_FEATURE_FILE,sie_config_path, topn)
    print '[Extract Feature] '+cmd
    os.system(cmd)
    cmd = 'python ../../py_talos.py --dllpath="{}" --label=0 --samplepath="{}" --out="{}" --configpath="{}" --topn="{}" '.format(DLL_PATH, NOR_SAMPLE_DIR, NOR_FEATURE_FILE,sie_config_path, topn)
    print '[Extract Feature] '+cmd
    os.system(cmd)
