...
function searchBuffer( buf, sig, startPos ) {
	if ( startPos < 0 || buf.length < sig.length ) return -1;

	for ( var pos = startPos; pos < buf.length - sig.length; ++ pos ) {
		var index = 0;
		for ( ; index < sig.length; ++ index ) {
			if ( buf[pos + index] != sig[index] ) break;
		}

		if ( index == sig.length ) return pos;
	}
...
Hzip.prototype.getEntry0304 = function(tmp,entry){
	...
	if ( entry.generalPurpose[0] & 0x08 ) {
		
		var descriptorSig = new Buffer( 4 );
        descriptorSig.writeUInt32LE( 0x08074b50, 0 );
        var sigIndex = searchBuffer( buffer, descriptorSig, tmp );

        sigIndex += 4; 
        entry.crc32 = buffer.slice( sigIndex, sigIndex + 4 );
		sigIndex += 4;
		entry.cfileSize = buffer.slice( sigIndex, sigIndex + 4 ).readInt32LE( 0 );
		sigIndex += 4;
		entry.fileSize = buffer.slice( sigIndex, sigIndex + 4 ).readInt32LE( 0 );
