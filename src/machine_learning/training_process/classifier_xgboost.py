...
class TMXGBTrainer(TrainerInterface):
    ...
    def train(self):
        best_params = {'learning_rate':0.6, 'max_depth':4, 'n_estimators':200}
        cpu_cores = multiprocessing.cpu_count()
        print 'The number of cpu cores is %d' % cpu_cores
        if self.config_['train']['search_best_params']:
            # searching best parameters
            param_dist = {'objective':'binary:logistic', 'n_jobs':cpu_cores}
            ...
            gs = GridSearchCV(estimator=xgb_model, param_grid=test_params, cv=5, verbose=10,\
                                 scoring = f2_score, n_jobs=cpu_cores)
            # # use auc as scoring
            # gs = GridSearchCV(estimator=xgb_model, param_grid=test_params, cv=5, verbose=10,\
