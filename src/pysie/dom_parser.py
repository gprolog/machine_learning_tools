...
class DOMParser:
    ...
    def __init__(self, config):
        self.config_ = config
        self.dom_tag_list_ = self.config_['dom_tag_list'].split(',')
        self.soup_ = None
    ...
    def parse_content(self, content):
        ...
        for child in self.soup_.recursiveChildGenerator():
            ...
            if name is not None:
                ...
                if child not in self.js_content_list        \
    ...
    def get_dom_features(self):
        ...
        for tag_name in ordered_map_tag_num:
            ...
            if value >= base:
                dom_features[dom_tag_base_index + index] = 1
            else:
                dom_features[dom_tag_base_index + index] = float(value) / base
            index += 1

        # tag relation statistic
        if self.config_['enable_dom_relation']:
                ...
                for tag_rel_name in ordered_map_tag_rel:
                    if tag_rel_name in self.dom_tag_relation_base_.keys():
                        ...
                        pass
                    index += 1
                dom_features.update(dom_tag_rel_feature)
        else:
            print( 'disable dom relation ship feature extraction...')
            ...
            dom_features.update(hist_features)

            # update other features
            dom_features[self.config_['dom_feature_index']['external_link_script_div_50']] = float(self.num_external_link_script_) / 50
            dom_features[self.config_['dom_feature_index']['internal_script_div_20']] = float(self.num_internal_script_) / 20
            return dom_features
